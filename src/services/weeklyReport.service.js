...
class WeeklyReportService {
  ...
  // Sinh viên sửa nội dung nhật ký (chỉ khi chưa duyệt)
  static async updateContent(id, content) {
    ...
    if (report.status === "đã duyệt") {
      throw new Error("Báo cáo đã được duyệt, không thể chỉnh sửa nữa.");
    }

    report.content = content;
    report.status = "chờ duyệt"; // Đặt lại về chờ duyệt khi sinh viên sửa
    return await report.save();
  }

  // Xoá nhật ký (chỉ khi chưa duyệt)
  static async delete(id) {
    ...
    if (report.status === "đã duyệt") {
      throw new Error("Báo cáo đã được duyệt, không thể xóa.");
    }
